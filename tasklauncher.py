...
def getWindowList():
    stdout, stderr = Popen(
        ['tmux list-windows -t ' + session_special], stdout=PIPE, shell=True).communicate()
    stdout = str(stdout)
    if "[" not in stdout:
        return []
...
def main():
    if sys.argv[1] == "ls":
        ...
        if "@" not in sys.argv[1]:
            print(
                "Wrong format. Needed: SESSION_NAME@ClUSTER[gID][#NUMGPUs]")
            exit()
        ...
        sshfs_cmd = "ssh " + cluster + " -t \"mkdir -p " + target + \
            "; nohup sshfs -o StrictHostKeyChecking=no -o follow_symlinks -o cache=no -o IdentityFile=~/.ssh/id_rsa " + \
            user_host + ":/ " + target + "\""
        cmd(sshfs_cmd)

        tf_cmd = "CUDA_VISIBLE_DEVICES=" + \
            gpu_id + " " + " ".join(sys.argv[2:])
        ...
        if len(windows) == 0:
            ...
            tmux_creation = 'tmux new -A -s ' + session_special + \
                '\; new-window -t ' + session_special + ' -n ' + session_name + ' \;'

        # https://unix.stackexchange.com/questions/266866/how-to-prevent-ctrlc-to-break-ssh-connection/841125
        terminal_cmd = ' ssh ' + cluster + ' -t \\\"trap : INT; ' + terminal_cmd + ' ; echo \\\"' + tf_cmd + \
            '\\\" >> ~/.zsh_history; /bin/zsh \\\"; exit'  # last exit is for when closing ssh connection, also close ROG
